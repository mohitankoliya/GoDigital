...
class restaurants {
    ...
    static changeOrderStatus = async (req, res) => {
        try {
            ...
            if (req.body.orderStatus === 'success') {
                var update = { $set: { status: req.body.orderStatus, end_time: new Date() } }
            } else {
    ...
    static todayOrderDetails = async (req, res) => {
        try {
            ...
            // var i = new Date(new Date().getFullYear() + '-' + (new Date().getMonth()+1)  + '-' + new Date().getDate() + ' 00:00:00')
            ...
            let dateWise = new Date(req.query.date)
    ...
    static subcription = async (req, res) => {
        try {
            ...
            if (pendingSub.length >= 1) {
                ...
                var someDate = runningSub[runningSub.length - 1] ? new Date(runningSub[runningSub.length - 1]) : new Date(new Date().getFullYear() + '-' + (new Date().getMonth() + 1) + '-' + new Date().getDate() + ' 00:00:00')
            }

            var endTime = someDate.setDate(someDate.getDate() + (28 * parseInt(req.body.month)))

            const savesData = subscription({
                start_date: runningSub[runningSub.length - 1] ? runningSub[runningSub.length - 1] : new Date(new Date().getFullYear() + '-' + (new Date().getMonth() + 1) + '-' + new Date().getDate() + ' 00:00:00'),
                end_date: endTime,
    ...
    static qrGenerator = async (req, res) => {
        try {
            ...
            const restaurantlink = 'http://' + req.headers.host + "/users/restaurant_menu?restaurant_id=" + req.decodedToken._id
            
            const tablearray = []
            for (let i = 0; i < queryFire.restaurant_type.dining_table.length; i++) {
                const element = queryFire.restaurant_type.dining_table[i];
                const dininglink = 'http://' + req.headers.host + "/users/restaurant_menu?restaurant_id=" + req.decodedToken._id + '&dining_number=' + element.dining_table_id;
                const serveData = {
            ...
            for (let i = 0; i < queryFire.restaurant_type.rooms.length; i++) {
                ...
                const dininglink = 'http://' + req.headers.host + "/users/restaurant_menu?restaurant_id=" + req.decodedToken._id + '&room_number=' + element.room_id;
                const serveData = {
