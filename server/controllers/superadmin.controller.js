...
class superAdmin {
        ...
        try {
            ...
            for (let j = 0; j < allsubcriptionData.length; j++) {
                ...
                if ((allsubcriptionData[j].subcription_running === true &&
                    ...
                    allsubcriptionData[j].start_date < new Date() &&
                    allsubcriptionData[j].end_date > new Date())) {
    ...
    static createRestAccount = async (req, res) => {
        try {
            ...
            var someDate = new Date(new Date().getFullYear() + '-' + (new Date().getMonth() + 1) + '-' + new Date().getDate() + ' 00:00:00');
            var endTime = someDate.setDate(someDate.getDate() + 28)

            const savesData = subscription({
                start_date: new Date(new Date().getFullYear() + '-' + (new Date().getMonth() + 1) + '-' + new Date().getDate() + ' 00:00:00'),
                end_date: endTime,
